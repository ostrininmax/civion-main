...
import type { ProcessInstance } from '../processes/processes.types';
import type { TimelineEvent } from '../timeline/timeline.service';
import type { WalletDocument } from '../wallet/wallet.types';

@Injectable()
export class DemoDataService {
  private seedLock: Promise<string> | null = null;
  ...
  private readonly fallbackDocuments: WalletDocument[] = [
    {
      id: 'doc-passport',
      category: 'passport',
  ...
  private readonly fallbackProcesses: ProcessInstance[] = [
    {
      id: 'proc-tax-id',
      processId: 'tax-id',
      status: 'completed',
  ...
  private readonly fallbackTimeline: TimelineEvent[] = [
    {
      id: 'tl-ltd-decision',
  ...
  private readonly fallbackConsents = [
    {
      id: 'consent-doc-core',
  ...
  async ensureGovernmentPrefilledResident() {
    if (this.seedLock) return this.seedLock;
    ...
    const runner = async () => {
      try {
        ...
        this.logger.error('Database seeding unavailable. Falling back to in-memory demo data.', error as Error);
  ...
  private async seedDocuments(ownerId: string) {
  ...
  private async seedProcesses(ownerId: string) {
  ...
  private async seedTimeline(ownerId: string) {
  ...
  private async seedConsents(ownerId: string) {
    ...
    const signature = (scope: string[], status: string) =>
      `${status}:${[...scope].sort((a, b) => a.localeCompare(b)).join('|')}`;
